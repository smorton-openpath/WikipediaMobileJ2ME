...
public class DefaultLookAndFeel extends LookAndFeel implements FocusListener {
    ...
    public void drawTextArea(Graphics g, TextArea ta) {
        ...
        //RJH-clear out textarealinks, we're gonna see if they should be drawn again
        for (int ii = 0; ii < ta.getTotalLinks(); ii++) {
            TextArea.TextAreaLink ll = (TextArea.TextAreaLink) ta.getLink(ii);
            ll.reset();
        }
        ...
        for (int i = 0; i < line; i++) {
            ...
            if(Rectangle.intersects(x, y, ta.getWidth(), fontHeight, oX, oY, oWidth, oHeight)) {
                ...
                //RJH- draw the links here on top of where we drew the line
                drawLinks(g, ta, displayText, i, f, x, y);
                shouldBreak = true;
            }else{
                if(shouldBreak){
                    break;
    ...
    //RJH- DrawLinks draw the link words on top in blue
    public void drawLinks(Graphics g, TextArea ta, String displayText, int i, Font f, int x, int y) {
        int oldcol = g.getColor();
        g.setColor(0x0000FF);
        
        //if the current line has the link inside, draw on top of where it is a blue text of the same letters
        for (int ii = 0; ii < ta.getTotalLinks(); ii++) {
            TextArea.TextAreaLink ll = (TextArea.TextAreaLink) ta.getLink(ii);
            //System.out.println("drawing link: " + ll.text);
            int lineStart = ((Integer) ta.rowIndexs.elementAt(i)).intValue();
            int lineEnd = lineStart + displayText.length();
            //if begin or end index is inside this line, we should draw
            if (ll.beginIndex <= lineEnd && ll.endIndex >= lineStart) {
                //System.out.println("link is" + ll.getText() + " | displaytext=" + displayText + " | ll.beginIndex=" + ll.beginIndex);

                int indexStarting = 0;
                //if we havent drawn any chars yet, we might not be at the start of the line
                if (ll.drawnChars == 0) {
                    //so figure out how far into the current line we start
                    indexStarting = (ll.beginIndex - lineStart);
                    //if we havent drawn the 1st or 2nd line, we need to increment our drawchars that would have drawn
                    if (indexStarting < 0) {
                        //and increment our drawn chars
                        ll.drawnChars -= indexStarting;
                        indexStarting = 0;
                    }
                }
                //get our x position to draw link by finding out how much is drawn so far in the line
                String notOurLink = displayText.substring(0, indexStarting);
                int startx = x + f.stringWidth(notOurLink);
                
                //get rid of all spaces before the link we're drawing
                String toDraw = ll.getText().substring(ll.drawnChars);
                while(toDraw.startsWith(" ")) {
                    toDraw = toDraw.substring(1);
                    ll.drawnChars++;
                }
                
                int totalCharactersCanDrawMax = displayText.length() - indexStarting;
                int howManyToDraw = ll.getText().length() - ll.drawnChars;
                if (howManyToDraw > totalCharactersCanDrawMax) {
                    howManyToDraw = totalCharactersCanDrawMax;
                }

                //get text to draw
                toDraw = ll.getText().substring(ll.drawnChars, ll.drawnChars + howManyToDraw);
                ll.drawnChars += howManyToDraw;
                
                int width = f.stringWidth(toDraw);
                int absx = g.getTranslateX() + startx;
                int absy = g.getTranslateY() + y;
                //System.out.println(ta + "drawing " + toDraw + " at y=" + absy + " x=" + absx + " w=" + width);
                Rectangle rect = new Rectangle(absx, absy, width, f.getHeight());
                ll.addClickable(rect);
                ll.isVisible = true;
                g.drawString(toDraw, startx, y, ta.getStyle().getTextDecoration());
                //System.out.println("ta.linkIndexCur=" + ta.linkIndexCur);
                if(ii == ta.linkIndexCur)
                    g.drawLine(startx, y+f.getHeight(), startx+width, y+f.getHeight());
            }
        }
        g.setColor(oldcol);
    }
