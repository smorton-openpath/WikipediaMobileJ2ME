...
public class Form extends Container {
    ...
    /**
     * Title area manipulation might break with future changes to LWUIT and might 
     * damage themeing/functionality of the LWUIT application in some platforms
     * 
    ...
    public void setFocused(Component focused) {
        ...
        //RJH-going down to focused, 
        //make focused go to the next link
        //make old focus go to the max(so it doesnt draw on last one)
        if(this.focused != null && this.focused.getNextFocusUp() == oldFocus) {
            if(this.focused instanceof TextArea) {
                ((TextArea)this.focused).setFocusToNextLink();
            }
            if(oldFocus != null && oldFocus instanceof TextArea) {
                ((TextArea)oldFocus).linkIndexCur = ((TextArea)oldFocus).getTotalLinks();
            }
        }
        //RJH-going up to focused
        //make focused go to the one less than max
        //make old focus go to -1
        if(this.focused != null && this.focused.getNextFocusDown() == oldFocus) {
            if(this.focused instanceof TextArea) {
                ((TextArea)this.focused).linkIndexCur = ((TextArea)this.focused).getTotalLinks()-1;
            }
            if(oldFocus != null && oldFocus instanceof TextArea) {
                ((TextArea)oldFocus).linkIndexCur = -1;
            }
        }
    }
    ...
    private boolean changeFocusState(Component cmp, boolean gained){
        //System.out.println("focus " + ((gained) ? "gained" : "lost") + cmp);
    ...
    public void keyReleased(int keyCode) {
        ...
        if (focused != null) {
            if (focused.getComponentForm() == this) {
                ...
                //RJH-if its got links and its a textarea, its not enabled, so let the keyrelease come in
                else if(focused instanceof TextArea){
                    if(((TextArea)focused).getTotalLinks() > 0){
                        focused.keyReleased(keyCode);
                    }
                }
            }
        ...
        if (y >= contentPane.getY()) {
            ...
            if(cmp != null) {
                ...
                if(cmp.hasLead) {
                    ...
                    //RJH-if its got links and its a textarea, its not enabled, so let the pointerPressed come in
                    else if (cmp instanceof TextArea) {
                        if (((TextArea) cmp).getTotalLinks() > 0) {
                            cmp.pointerPressed(x, y);
                        }
                    }
                }
    ...
    public void pointerReleased(int x, int y) {
        ...
        if (dragged == null) {
            ...
            if (y >= contentPane.getY()) {
                ...
                //RJH-if its got links and its a textarea, its not enabled, so let the pointerReleased come in
                else if (cmp instanceof TextArea) {
                    if (((TextArea) cmp).getTotalLinks() > 0) {
                        cmp.pointerReleased(x, y);
                    }
    ...
    private void updateFocus(int gameAction) {
        ...
        Component startFocus = focused;
        Component down = null, up = null;
        boolean dontscroll = false;
        switch (gameAction) {
            case Display.GAME_DOWN: {
                down = findNextFocusDown();
                if(down != null) {
                    if (down != focused) {
                       dontscroll = true;
                    }
        ...
        if (isFocusScrolling()) {
            ...
            //RJH-if we're going down, try to set the focused textarea's link to the next one
            //setFocusToNextLink will only do so, if the next one is visible
            if (down == startFocus) {
                if (startFocus instanceof TextArea) {
                    if (((TextArea) startFocus).getTotalLinks() > 0) {
                        ((TextArea) startFocus).setFocusToNextLink();
                    }
                }
            //RJH-if we're going up, try to set the focused textarea's link to the prev one
            //setFocusToPrevLink will only do so, if the next one is visible
            } else if (up == startFocus) {
                if (startFocus instanceof TextArea) {
                    if (((TextArea) startFocus).getTotalLinks() > 0) {
                        ((TextArea) startFocus).setFocusToPrevLink();
                    }
                }
            }
            ...
            if (moveScrollTowards(gameAction, focused)) {
                setFocused(focused); 
                //RJH-dont scroll if we're switching focused elements, otherwise we'll scroll to the top of the element and itll look weird
                if(dontscroll == false)
                    scrollComponentToVisible(focused);
            }
        }
