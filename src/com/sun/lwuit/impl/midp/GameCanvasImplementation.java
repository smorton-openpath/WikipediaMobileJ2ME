...
public class GameCanvasImplementation extends LWUITImplementation {
        ...
        public void setCommands(Vector v) {
            ...
            for(int iter = 0 ; iter < currentCommands.length ; iter++) {
                ...
                if(current == backCommand) {
                    ...
                    if(iter == 1 && thirdSoftButton) {//Axthelm 7/26/12 - Adding thirdSoftButton so that Nokia Asha and s60 phones can control the center softkey.
                        currentCommands[iter] = new MIDP2LWUITCommand(current, Command.OK, iter + 1);
                    } 
                    else if(iter == 0 ) {
                        currentCommands[iter] = new MIDP2LWUITCommand(current, Command.EXIT, iter + 1);
                    } 
                    else {
    ...
    private void setSoftKeyCodes(MIDlet m) {
        ...
        tmpSoftKey = m.getAppProperty("SoftKey-Center");
        if (tmpSoftKey != null && !"".equals(tmpSoftKey)) {
            centerSK[0] = Integer.valueOf(tmpSoftKey).intValue();
        }
...
// Check Third Soft Button is supported        
    ...
    public int getSoftkeyCount() {
        return 3;//Axthelm - will this break things?
    }
