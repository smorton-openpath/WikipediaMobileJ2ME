...
public class TextArea extends Component {
    ...
    /**
     * Defines a selectable link within a text area.
     */
    public static class TextAreaLink {
        private String text = "";
        private String link = "";
        public int drawnChars = 0;
        public Vector clickables = new Vector();
        public boolean isVisible = false;
        
        // The position of this link in the parent's text.
        public int beginIndex = 0, endIndex = 0;
        
        public TextAreaLink(String link, int beginIndex) {
            this.link = link;
            this.drawnChars = 0;
        ...
        public void reset() {
            this.drawnChars = 0;
            clickables = new Vector();
            isVisible = false;
        }
        
        public void addClickable(Rectangle rect) {
            clickables.addElement(rect);
        }
        
        public boolean isInBounds(int x, int y){
            for(int i = 0; i < clickables.size(); i++) {
                Rectangle rect = (Rectangle) clickables.elementAt(i);
                if(x >= rect.getX() && x <= rect.getX() + rect.getSize().getWidth() &&
                    y >= rect.getY() && y <= rect.getY() + rect.getSize().getHeight())
                    return true;
            }
            return false;
        }
    ...
    public Vector rowStrings, rowIndexs = new Vector();
    ...
    public boolean isFocusable() {
        if(this.getTotalLinks() > 0 && isVisible()) {
            return true;
        }
        else
    ...
    public void keyReleased(int keyCode) {
        ...
        //RJH-hand over to click
        else if(this.getTotalLinks() > 0 && keyCode == -5) {
            onClick();
        }
    }
    ...
    //RJH-overrode this so it stayed focused on this textarea if more links are available
    ...
    //RJH-overrode this so it stayed focused on this textarea if prev links are available
    ...
    void onClick(){
        //RJH-if theres links, open the current one
        if (this.getTotalLinks() > 0) {
            openLink(this.getLink(linkIndexCur));
        }
        if(isEditable()) {
            editString();
        }
    ...
    public void pointerReleased(int x, int y) {
        ...
        if (isDragActivated()) {
            ...
            super.pointerReleased(x, y);
            if (isEditable() && isEnabled() && !isCellRenderer() && getTotalLinks() == 0) {
                Display.getInstance().editString(this, getMaxSize(), getConstraint(), getText());
                if (Display.getInstance().isTouchScreenDevice()) {
                    if(Display.getInstance().getDefaultVirtualKeyboard() != null)
                    {
            ...
            //RJH-hand over the pointerrelease
            else
                handleLinkPress(x, y);
        }
    ...
    //RJH-pointer to the current index being highlighted
    public int linkIndexCur = -1;
    boolean hasMoreLinks() {
        if (this.getTotalLinks() == 0) {
            return false;
        }
        if (linkIndexCur + 1 > this.getTotalLinks()-1) {
            return false;
        }
        return true;
    }
    boolean hasPrevLinks() {
        if (this.getTotalLinks() == 0) {
            return false;
        }
        if (linkIndexCur - 1 < 0) {
    ...
    //RJH-sets focus to next link ONLY if its also visible on screen
    void setFocusToNextLink() {
        if(getTotalLinks() == 0) return;
        TextAreaLink tal = getLink(linkIndexCur+1);
        if(tal != null && !tal.isVisible) return;
        if(tal == null) return;
        linkIndexCur++;
        repaint();
    }
    //RJH-sets focus to prev link ONLY if its also visible on screen
    void setFocusToPrevLink() {
        if(getTotalLinks() == 0) return;
        TextAreaLink tal = getLink(linkIndexCur-1);
        if(tal != null && !tal.isVisible) return;
        if(tal == null) return;
    ...
    //RJH-added a rowIndex vector that keeps track of what index into the textarea each of the links text's are at
    private synchronized void initRowString() {
        Style style = getUnselectedStyle();
        rowStrings= new Vector();
        rowIndexs = new Vector();
        ...
        if ((isSingleLineTextArea()) || (widthForRowCalculations<=0)) {
            rowIndexs.addElement(new Integer(0));
        ...
        while(to<textLength) {
            ...
            //RJH-index of where the link text is at, for use in drawTextArea
            rowIndexs.addElement(new Integer(lastFrom + new String(text).substring(lastFrom).indexOf(rowText)));
        ...
        if(text[text.length -1 ] == '\n'){
            ...
            rowIndexs.addElement(new Integer(text.length-1));
        }
    }
    ...
    /**
     * Sets the list of links contains in the text area
     * 
     * @param list the vector of links included in the text
     */
    ...
    /**
     * Returns the existing list of links.
     */
    public TextAreaLink getLink(int index) {
        if (this.links.size() > index && index >= 0) {
            return (TextAreaLink) this.links.elementAt(index);
        }
        
        return null;
    }
    
    public void handleLinkPress(int x, int y) {
        for(int i = 0; i < links.size(); i++) {
            TextAreaLink tal = (TextAreaLink) links.elementAt(i);
            if(tal.isInBounds(x, y)){
                //System.out.println("OPEN LINK!!" + tal.link);
                openLink(tal);
                return;
            }
        }
    ...
    public void openLink(TextAreaLink link) {
        //System.out.println("clicked" + link.link);
        LinkButton lb = new LinkButton(link.text, link.link);
        Command fakeCommand = new Command("", com.pages.BasePage.COMMAND_LINK);
        ActionEvent ae = new ActionEvent(fakeCommand, lb, 0, 0);
        getComponentForm().actionCommandImpl(fakeCommand, ae);
    }
}
