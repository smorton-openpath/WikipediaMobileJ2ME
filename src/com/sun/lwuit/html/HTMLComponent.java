...
//    long textTime;
    ...
    private HTMLLink currLink = null;
    ...
    public static void addSpecialKey(String specialKeyName,int specialKeyCode) {
        if (SUPPORT_CSS) {
            ...
            System.out.println("HTMLComponent.addSpecialKey: LWUIT was compiled with HTMLComponent.SUPPORT_CSS false, this method has no effect");
        }
    }
    ...
    public static void setCSSSupportedMediaTypes(String[] supportedMediaTypes) {
        if (SUPPORT_CSS) {
            ...
            System.out.println("HTMLComponent.setCSSSupportedMediaTypes: LWUIT was compiled with HTMLComponent.SUPPORT_CSS false, this method has no effect");
        }
    }
    ...
    private Label addString(String str,int align) {
        ...
        if (link!=null) {
            //System.out.println("Link String: "+str);
            if(currLink != null)
            {
                currLink.setText(currLink.getText() + str);
                lbl = currLink; 
                newLbl = false;
            }else
            {
                lbl=new HTMLLink(str,link,this,mainLink,linkVisited);
                currLink = (HTMLLink)lbl;
            }
            color=linkColor;
            if (linkVisited) {
    ...
    private void handleInput(HTMLElement element,int align) {
        ...
        switch(typeID) {
            ...
            case INPUT_SUBMIT:
                ...
                if (submitCmd==null) {
                    submitCmd=new Command(value.equals("")?HTMLForm.getDefaultSubmitText():value); //dummy command - no form so it won't do anything
                }
        ...
        for(int i=0;i<element.getNumChildren();i++) {
            ...
            switch (child.getTagId()) {
                ...
                case HTMLElement.TAG_SELECT:
                    ...
                    // If a select tag has size specified, it will be shown as an open list, and not as LWUIT combo, even if there's no multiple seection allowed
    ...
    class ForLabel extends Label {
        ...
        ForLabel(String labelText,HTMLComponent htmlC,String id) {
            super(labelText);
            //System.out.println("forLabel: "+labelText);
            this.id=id;
            this.htmlC=htmlC;
        }
